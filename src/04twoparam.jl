...
# try
    ...
    Dtree = dryad(M0, last(vrbl))
    # print_tree(Dtree)
    # apply_tree(Dtree, [.0, 1, 0])
    ...
    data = DataFrame(solve(f_, [eps(), .05853, .47898], dt, 0:dt:10, Dtree), last(vrbl))
